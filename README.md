...
Дополнительно для `Notifications -> Deleted`:
- пользователь может открыть связанный удалённый контент по ссылке из уведомления, подать `appeal`, если он доступен на текущей ступени, и удалить само уведомление у себя
